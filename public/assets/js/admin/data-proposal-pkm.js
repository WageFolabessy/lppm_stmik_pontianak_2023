...
$("#tabelDataProposalPKM").DataTable({
    processing: true,
    serverSide: true,
    ajax: "/daftar_proposal_pkm/datatables",
    columns: [
...
$(document).on("click", "#tombol-detail", function (e) {
    ...
    $.ajax({
        url: "/daftar_proposal_pkm/detail/" + id,
        type: "GET",
...
function tambahKomentar(id) {
    $.ajax({
        url: "/daftar_proposal_pkm/tambah_komentar/" + id,
        type: "POST",
        data: {
...
function terimaProposal(id) {
    $.ajax({
        url: "/daftar_proposal_pkm/terima_proposal/" + id,
        type: "POST",
        data: {
            _token: $('meta[name="csrf-token"]').attr("content"),
        },
...
function tolakProposal(id) {
    $.ajax({
        url: "/daftar_proposal_pkm/tolak_proposal/" + id,
        type: "POST",
        data: {
            _token: $('meta[name="csrf-token"]').attr("content"),
        },
...
$(document).on("click", "#tombol-hapus", function (e) {
    ...
    if (confirm("Apakah Anda yakin ingin menghapus proposal PKM ini?")) {
        $.ajax({
            url: "/daftar_proposal_pkm/hapus_proposal/" + id,
            type: "DELETE",
            data: {
                _token: $('meta[name="csrf-token"]').attr("content"),
            },
